...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(typeof createMenu(objetoRetornado)).toBe('object');
    expect(createMenu(objetoRetornado)).toHaveProperty('fetchMenu');
    expect(typeof createMenu(objetoRetornado).fetchMenu).toBe('function');
    
    // TESTE 2: Verifique se 'objetoRetornado.fetchMenu()' retorna um objeto cujas chaves são somente `food` e `drink`, 
    // considerando que a função createMenu() foi chamada com o objeto: `{ food: {}, drink: {} }`.
    // ```
    // const objetoRetornado = createMenu({ food: {}, drink: {} });objetoRetornado
    ...
    // ```
    let auxiliar= createMenu({food: {}, drink: {}});
    auxiliar.order('coxinha');
    expect(auxiliar.consumption).toEqual(['coxinha']);
    ...
    // ```
    auxiliar = createMenu({food: {}, drink: {}});
    auxiliar.order("coxinha");
    auxiliar.order("agua");
    auxiliar.order("sopa");
    auxiliar.order("sashimi");
    expect(auxiliar.consumption).toEqual(['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    auxiliar.order("agua");
    auxiliar.order("coxinha");
    expect(auxiliar.consumption).toEqual(['coxinha', 'agua', 'coxinha']);
    ...
    auxiliar.order('coxinha');
    auxiliar.order('caipirinha');
    auxiliar.order('coxinha');
    expect(auxiliar.pay()).toBe(60);
